...
class NTMCell(tf.contrib.rnn.RNNCell):
    def __init__(self, controller_layers, controller_units, 
                 memory_size, memory_vector_dim,              # 存储单元
                 read_head_num, write_head_num,               # 读写头数目
                 addressing_mode='content_and_location', 
                 shift_range=1,                # 基于地址的寻址中可移位的数目
                 reuse=False, output_dim=None, clip_value=20,  # reuse=False
                 init_mode='constant'):
        ...
        # 存储单元 M 的初始化方式为 “constant”，全部初始化为1e-6
    ...
    def __call__(self, x, prev_state):
        # 上一个时间步的read结果，当前时间步的输入，混合起来，作为RNN单元的输入
        ...
        # 上一个时间步的权重，用于和当前计算出的权重进行插值
        prev_w_list = prev_state.w_list
        prev_M = prev_state.M              # 存储单元
        w_list = []
        # 循环处理每个读写头
        ...
        # Reading (Sec 3.1)
        # 根据权重 w_list 权重，对存储单元内容 prev_M 进行读取
        read_w_list = w_list[:self.read_head_num]
        read_vector_list = []
        for i in range(self.read_head_num):
            read_vector = tf.reduce_sum(tf.expand_dims(read_w_list[i], axis=2) * prev_M, axis=1)
            read_vector_list.append(read_vector)  # 多个读取头读取的内容append到一起

        # Writing (Sec 3.2)
        # 根据权重 w_list 以及 et 和 at 内容，对相应单元进行写入。输出的M为新的存储单元内容
        ...
        # 最后对 读取头 读取的内容read_vector_list 和 controller原来的输出连接起来
        # 使用全连接网络进行输出，随后进行clip.
